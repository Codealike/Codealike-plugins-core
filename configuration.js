...
var baseGlobalSettings = {
  userToken: null,
  apiUrl: "http://3.235.109.156/Codealike/api/v2",
};
...
var Configuration = {
  ...
  loadPluginSettings: function (settings) {
    Configuration.pluginSettings = Object.assign(
      {},
      basePluginSettings,
      settings
    );
  ...
  loadGlobalSettings: function () {
    ...
    if (fs.existsSync(codealikeSettingsFile)) {
      let existingConfiguration = JSON.parse(
        fs.readFileSync(codealikeSettingsFile, "utf8")
      );
  ...
  savelGlobalSettings: function (settings) {
    ...
    fs.writeFile(codealikeSettingsFile, jsonString, "utf8", function (error) {
      if (error) {
